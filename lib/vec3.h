...
public:
	...
	__device__ __host__ inline vec3 clamp();    
...
__device__ __host__ inline vec3 vec3::clamp()
{
	float rr,gg,bb;
	rr=r();
	gg=g();
...
__device__ __host__ inline  vec3 lerp(const vec3& u, const vec3& v, float t)
{
	return (1.0 - t) * u + t * v;
}
...
__device__ __host__ inline  bool refract(const vec3&v, const vec3&n, float ni_over_nt, vec3& refracted)
{
...
__device__ __host__ inline  vec3 random_in_unit_sphere()
{
    vec3 p;
    do {
